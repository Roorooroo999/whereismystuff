...
AS (
  ...
  DATES AS (
    ...
    WHERE DC.RDC_IND = 1 OR DC.FDC_IND = 1
       ...
       OR DC.ICC_IND = 1   -- Import Consolidation Centers → DC_TYPE_DESC='MCC/ACC' → ON_YARD_MCC_UNITS
  ),
...
AS (
  ...
  on_yard_agg AS (
    SELECT
      ...
      SUM(CASE WHEN DC_TYPE = 'MCC/ACC'               THEN ON_YARD_UNITS ELSE 0 END) AS ON_YARD_MCC_UNITS,
  ...
  SELECT
    ...
    COALESCE(oy.ON_YARD_MCC_UNITS, 0)      AS ON_YARD_MCC_UNITS,
