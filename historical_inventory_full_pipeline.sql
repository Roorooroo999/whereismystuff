...
AS (
  ...
  DATES AS (
    ...
    WHERE DC.RDC_IND = 1 OR DC.FDC_IND = 1
       OR DC.GDC_IND = 1   -- Grocery DCs → DC_TYPE_DESC='Grocery' → ON_YARD_GROCERY_UNITS
       OR DC.ICC_IND = 1   -- Import Consolidation Centers → DC_TYPE_DESC='MCC/ACC' → ON_YARD_ICC_UNITS
  ),
...
AS (
  ...
  on_yard_agg AS (
    SELECT
      ...
      SUM(CASE WHEN DC_TYPE = 'MCC/ACC'               THEN ON_YARD_UNITS ELSE 0 END) AS ON_YARD_ICC_UNITS,  -- ICC = Import Consolidation Centers (DC_TYPE_DESC='MCC/ACC')
      SUM(CASE WHEN DC_TYPE NOT IN ('Regional','Grocery','Fashion','Imports','GIDC','Market Service Center','MCC/ACC')
  ...
  SELECT
    ...
    COALESCE(oy.ON_YARD_ICC_UNITS, 0)      AS ON_YARD_ICC_UNITS,
